# -*- coding: utf-8 -*-
"""
Created on Wed Feb 23 12:53:36 2022
...
if 'PYCONTROL_TEST_EXAMPLES' not in os.environ:
    plt.show()
"""
# Подход 1: стандартная квадратичная стоимость
# Мы можем сформулировать задачу оптимального управления как попытку минимизировать
расстояние от желаемой конечной точки, в то же время не прилагая слишком много усилий 
по управлению для достижения нашей цели.
# Подход 2: входные затраты, входные ограничения, терминальные затраты
# Предыдущее решение интегрирует ошибку положения для всего горизонта, поэтому 
автомобиль очень быстро меняет полосу движения (за счет больших входных данных). 
Вместо этого мы можем оштрафовать конечное состояние и увеличить стоимость 
входных данных, что приведет к более плавному изменению дорожки. Мы также 
устанавливаем решатель явно.
# Подход 3: терминальные ограничения
# Мы также можем удалить функцию стоимости состояния и заменить ее терминальным 
*ограничением* состояния. Если решение найдено, оно гарантирует, что мы попадем 
точно в конечное состояние.
# Подход 4: терминальные ограничения с базисными функциями
# В качестве последнего примера мы можем использовать базовую функцию, чтобы 
уменьшить размер задачи и получить более быстрые ответы с большим временным 
разрешением. Здесь мы параметризуем входные данные набором из 4 кривых Безье, 
но решаем для гораздо большего набора входных данных с временным разрешением."""